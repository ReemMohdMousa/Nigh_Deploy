...
const FriendRequests = ({ id }) => {
  ...
  const checkIfReqWasSent = () => {
    axios
      .get(`https://nigh-deploy.onrender.com/friends/sent/requests`, {
  ...
  const checkIfReqWasReceived = () => {
    axios
      .get(`https://nigh-deploy.onrender.com/friends/received/requests`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(function (response) {
  ...
  const addFriendFun = (id) => {
    axios
      .post(
        `https://nigh-deploy.onrender.com/friends/add`,
        { user2_id: id },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const acceptFriendReq = () => {
    axios
      .post(
        `https://nigh-deploy.onrender.com/friends/accept`,
        { user2_id: id },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      .then(function (response) {
        ...
        // axios
        //   .get(`https://nigh-deploy.onrender.com/users/others/info/${friendId}`)
        //   .then((response) => {
        //     console.log(response.data.result);
  ...
  const cancelFriendReqFun = () => {
    axios
      .delete(`https://nigh-deploy.onrender.com/friends/cancel/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(function (response) {
        response.data.result.map((element, i) => {
  ...
  const declineFriendReqFun = () => {
    axios
      .delete(`https://nigh-deploy.onrender.com/friends/decline/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(function (response) {
        // console.log(response.data.result);
